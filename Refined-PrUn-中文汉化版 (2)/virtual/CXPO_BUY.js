...
const CXPO_BUY = act.addActionStep({
  ...
  description: data => {
    ...
    if (!willFillCompletely && allowUnfilled) {
      let description2 = `Bid for ${fixed0(data.amount)} ${ticker} on ${exchange}`;
      if (isFinite(priceLimit)) {
        description2 += ` at price ${fixed02(data.priceLimit)}`;
        description2 += ` (${fixed0(data.amount * data.priceLimit)} total cost)`;
      }
      return description2;
    }
